...
if (!empty($requestPOST)) {
    ...
    $personTypeId = $request->post('PERSON_TYPE_ID');
    ...
    if (!$hasErrors) {
        ...
        if (!$USER->IsAuthorized()) {
            ...
            if ($userId === null) {
                $captcha = COption::GetOptionString(
                    'main',
        ...
        if (!$hasErrors) {
            ...
            if (!$productId) {
                ...
                while ($row = $saleBasket->fetch()) {
                    $backToBasket[$row['ID']] = $row['QUANTITY'];
                    CSaleBasket::Delete($row['ID']);
                }
                $added = Add2BasketByProductID($productId, 1, array(), false);
            }
...
if (!empty($arParams['PRODUCT_ID'])) {
    $element = CIBlockElement::GetByID($arParams['PRODUCT_ID']);
