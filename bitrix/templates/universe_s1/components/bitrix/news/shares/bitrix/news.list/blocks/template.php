        ...
        <div class="block-element">
            <div class="block-element-wrapper" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
                <div class="block-element-link intec-cl-text-hover">
                    <a href="<?= $arItem["DETAIL_PAGE_URL"] ?>" class="block-element-img" style="background-image: url('<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>')">
                        <? if ($sale) { ?>
                    ...
                    <a class="element-text-name" href="<?= $arItem["DETAIL_PAGE_URL"] ?>">
                        <?= $arItem['NAME'] ?>
                    </a>
                </div>
                <?php if ($period) {?>
