...
<?php if (!defined('EDITOR')) { ?>
    ...
    $('#<?= $arResult['COMPONENT_HASH'] ?> [data-product-id]').on('changeQuantity', function(e, value){
        var productId = $(this).data('product-id'),
            $input = $('.quantity-value', this),
            maxQuantity = parseFloat($(this).data('max-quantity')),
        ...
        clearTimeout(window.ajaxTimeout_<?= $arResult['COMPONENT_HASH'] ?>);
        window.ajaxTimeout_<?= $arResult['COMPONENT_HASH'] ?> = setTimeout(function(){
            var data = {
                id: productId,
                quantity: value
            };
            universe.basket.setQuantity(data);
        }, 1000);
    });

    $('#<?= $arResult['COMPONENT_HASH'] ?>').on('click', '.quantity-up, .quantity-down', function(){
        var $wrapper = $(this).closest('[data-product-id]'),
            $input = $(this).siblings().filter('input.quantity-value'),
        ...
        $wrapper.trigger('changeQuantity', value);
    });

    $('#<?= $arResult['COMPONENT_HASH'] ?>').on('change', '.quantity-value', function(){
        var $input = $(this),
            $wrapper = $(this).closest('[data-product-id]'),
            value = parseFloat($input.val()).toFixed(2);
