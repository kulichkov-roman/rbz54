...
if (!in_array(true, array_values($arResult['SHOW_BLOCK'])) && !defined('EDITOR')) {
    return;
}
...
$openAfterAdd = ArrayHelper::getValue($arParams, 'OPEN_AFTER_ADD');
if ($openAfterAdd == 'settings') {
    $build = Build::getCurrent();
    $page = $build->getPage();
    $properties = $page->getProperties();

    switch ($properties->get('show_flying_basket_when_add_product')) {
        case 0:
            $openAfterAdd = 'N';
            break;
        case 1:
...
$arParams['OPEN_AFTER_ADD'] = ArrayHelper::fromRange(['N', 'Y'], $openAfterAdd);
unset($openAfterAdd);
                ...
                <div class="flying-basket_content flying-basket_content_basket <?= $arParams['ACTIVE_TAB'] === 'flying-basket_content_basket' ? 'show' : '' ?>">
                    <?php if ($arResult['BASKET_COUNT'] > 0) { ?>
                        <div class="flying-basket_content_header">
                            <span class="flying-basket_clear intec-button intec-button-cl-default intec-button-md intec-button-transparent">
                        ...
                        <div class="flying-basket_sum_wrapper">
                            ...
                            <span class="flying-basket_sum_value"><?= number_format($arResult['BASKET_SUM'], 2, '.', ' ') ?></span>
                        </div>
            ...
            if ($arResult['SHOW_BLOCK']['DELAYED']) { ?>
                <div class="flying-basket_content flying-basket_content_delayed <?= $arParams['ACTIVE_TAB'] === 'flying-basket_content_delayed' ? 'show' : '' ?>">
                    ...
                    <?php } else { ?>
                        ...
                        <div class="flying-basket_content_empty_wrapper intec-no-select">
                            <div class="flying-basket-content_empty_description"><?= GetMessage('WBF_DELAYED_EMPTY') ?></div>
                            <span class="flying-basket_close intec-button intec-button-cl-default intec-button-transparent intec-button-md">
                                <?= GetMessage('WBF_CLOSE') ?>
                            </span>
                        </div>
